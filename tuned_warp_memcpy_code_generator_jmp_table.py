...
(*_warp_memcpy_{T_sz}_table[{active_cnt}-1]) ( ({T_sz_generic_type}*) dest, ({T_sz_generic_type}*) src, prior_count, num);
"""
...
DEFINE_JUMPTABLE_TYPE_FORMAT_STRING = """
typedef void (*warp_memcpy_ptr_{T_sz}_t)({T_sz_generic_type}* __restrict__, const {T_sz_generic_type}* __restrict__, uint32_t, size_t);
"""
...
def generate_switch_table(T_sz_active_cnt_unroll_factor_tuples):
    ...
    result_str += "default: _warp_memcpy_general<T> ( dest, src, prior_count, num, active_cnt);return; //general fall back scheme\n" #general fall back scheme
    result_str+="} //end of switch T_sz \n" #end of switch T_sz
    return result_str

def generate_jump_table(T_sz):
    result_str=""
    for active_cnt in range(1,33):
        result_str+=("&"+DECLARE_WARP_MEMCPY_FORMAT_STRING.format(T_sz=T_sz,active_cnt=active_cnt)+",")
    return result_str[:-1] #last comma is redundant
if __name__ == "__main__":
    with open(TUNED_CODE_H,'w') as fd:
        ...
        # write jump table type
        T_szs = set([T_sz for (T_sz, active_cnt, unroll_factor) in T_sz_active_cnt_unroll_factor_tuples])
        for T_sz in T_szs:
            fd.write(DEFINE_JUMPTABLE_TYPE_FORMAT_STRING.format(T_sz=T_sz, T_sz_generic_type=T_sz_generic_type[T_sz]))

        # write jump table 
        for T_sz in T_szs:
            fd.write(DEFINE_JUMPTABLE_FORMAT_STRING.replace(r"{jump_table}",generate_jump_table(T_sz)).format(T_sz=T_sz))
